...
namespace SA
{
    ...
    class delegate<RET(PARAMS...)> final : private delegate_base<RET(PARAMS...)>
    {
      public:
        ...
        template <typename LAMBDA> delegate(const LAMBDA &lambda)
        {
        ...
        template <typename LAMBDA> // template instantiation is not needed, will
                                   // be deduced (inferred):
        ...
        {
            return delegate(const_cast<T *>(instance),
                            const_method_stub<T, TMethod>);
        } // create

        template <RET (*TMethod)(PARAMS...)> static delegate create()
        {
