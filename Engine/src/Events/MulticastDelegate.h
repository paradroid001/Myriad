        ...
        : private delegate_base<RET(PARAMS...)>
    {
      public:
        ...
        bool operator==(const multicast_delegate &another) const
        {
            ...
            for (auto it = invocationList.begin(); it != invocationList.end();
                 ++it)
                if (**it != **anotherIt)
                    return false;
            return true;
        ...
        multicast_delegate &operator+=(const multicast_delegate &another)
        {
            for (auto &item :
                ...
                this->invocationList.push_back(
                    new
                    typename delegate_base<RET(PARAMS...)>::InvocationElement(
                        item->object, item->stub));
            return *this;
        } // operator +=

        template <typename LAMBDA> // template instantiation is not neededm,
                                   // will be deduced/inferred:
                                   multicast_delegate &
