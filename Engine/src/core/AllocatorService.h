...
namespace Myriad
{
    ...
    bool AllocatorService::Shutdown()
    {
        ...
        for (const auto &[key, value] : _allocs_map)
        {
            ...
            // We can only delete because the handles have the BaseHandle type.
            // At the moment, handles still have a ref count of 1 because of
            // the fact that they are assigned here in this map.
            // We manually call their destructor, and they get decremented by
            // one.
            delete key;
        }
        ...
        // So handles have one more ref than they should because this allocator
        // holds them all, but even if they didn't, when they self destruct
        // the allocator will never know, unless the handles all know about
        // the allocator.
        MYR_CORE_WARN(
            "Fix the allocator, it will never know when handles are deleted.");
        return true;
    }
    ...
    MyrHandle<T> AllocatorService::Alloc(Args... args)
    {
        ...
        //    itself is never actually deleted because of the ref-count problem.
